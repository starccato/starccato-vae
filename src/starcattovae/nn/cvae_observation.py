...
class CVAE(nn.Module):
    def __init__(self, latent_dim, hidden_dim, input_dim, condition_dim, DEVICE):
        ...
        self.q = EncoderQ(signal_dim=input_dim, hidden_dim=hidden_dim, latent_dim=latent_dim, param_dim=condition_dim)
...
# only use signals, no conditioning
...
class EncoderQ(nn.Module):
    def __init__(self, signal_dim, hidden_dim, latent_dim, num_channels=1, kernel_size=3, stride=1, padding=1):
        super(Encoder, self).__init__()
        self.latent_dim = latent_dim
